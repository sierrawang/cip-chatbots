...
def get_messages_for_country(country, student_data, chat_messages):
    # Get the user_ids for students in this country
    user_ids = student_data[student_data['country'] == country]['user_id']
...
def output_latex_table(results_df):
    # Sort the data by average messages
    sorted_df = results_df.sort_values(by='avg_message_count', ascending=False)
    
    table_str = """\\begin{table}
\\centering
\\begin{tabularx}{\\columnwidth}{l X X l X X}
\\toprule
...
\\midrule"""
    ...
    for index, (_, row) in enumerate(sorted_df.iterrows()):
        if index == halfway:
            ...
            table_str += f"\n{row['country']} & {row['sent_message']:.1f}\\% & {row['avg_message_count']:.1f} & {second_half_row['country']} & {second_half_row['sent_message']:.1f}\\% & {second_half_row['avg_message_count']:.1f} \\\\"
        else:
            # Add the current row
    ...
    table_str += """\n\\bottomrule
\\end{tabularx}
\\caption{Shows chat usage by country --- specifically, the percent of students who sent a message to the chatbot (\\%) and the average number of messages that they sent (\\#). This table includes only students who received a chatbot in the IDE and countries with at least two students.}
